...
Public Class frmDeptMobile
    Private Const pxeMasTablex As String = "Department"
    Private Const pxeTableName As String = "Dept_Mobile"

    Private pnLoadx As Integer
    Private poControl As Control

    Private psDeptIDxx As String
    ...
    Private Sub frmDeptMobile_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub txtOther_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs)
        ...
        With DataGridView1
            ...
            If Mid(loTxt.Name, 1, 8) = "txtOther" Then
                Select Case loIndex
                    Case 1
                        poDT(lnRow)("sMobileNo") = txtOther01.Text
                        .Rows(lnRow).Cells(1).Value = poDT(lnRow)("sMobileNo")
                End Select
            End If
        End With
    End Sub
...
endwithRoll:
    ...
    Private Function BrowseRecord() As Boolean
        ...
        lsSQL = "SELECT" & _
                ...
                " FROM " & pxeMasTablex & " a" & _
                    " LEFT JOIN Employee_Master001 b" & _
                        " ON a.sDeptHead = b.sEmployID" & _
                    " LEFT JOIN Client_Master c" & _
                        " ON b.sEmployID = c.sClientID" & _
                " WHERE a.cRecdStat = " & strParm(xeRecordStat.RECORD_NEW) & _
                    " ORDER BY a.sDeptName ASC"
        ...
        With p_oAppDriver
            poRow = KwikSearch(p_oAppDriver _
                                            ...
                                            , "Department ID»Description" _
                                            , "a.sDeptIDxx»a.sDeptName" _
                                            , "a.sDeptIDxx»a.sDeptName" _
                                            , 1)
    ...
    Private Sub LoadDetail(Optional ByVal showonly As Boolean = False)
        ...
        lsSQL = "SELECT nEntryNox, sMobileNo FROM " & pxeTableName & _
                        " WHERE sDeptIDxx = " & strParm(poRow(0)) & _
                        " ORDER BY nEntryNox"
        ...
        If poDT.Rows.Count = 0 Then
            MsgBox("Department mobile number not set.", MsgBoxStyle.Information, "Notice")
            Call AddDetail()
        End If
...
loadrecord:
        With DataGridView1
            ...
            For lnCtr = 0 To lnRow - 1
                ...
                .Rows(lnCtr).Cells(1).Value = poDT(lnCtr)("sMobileNo")
            Next
        ...
        Try
            ...
            If poDTTemp.Rows.Count = 0 Then
                ...
                For lnCtr = 0 To poDT.Rows.Count - 1
                    If poDT(lnCtr)("sMobileNo") <> "" Then
                        lsSQL = "INSERT INTO " & pxeTableName & _
                                " SET nEntryNox = " & lnCtr + 1 & _
                                    ", sMobileNo = " & strParm(poDT(lnCtr)("sMobileNo")) & _
                                    ", sDeptIDxx = " & strParm(psDeptIDxx)
            ...
            Else
                If poDT.Rows.Count > 0 Then
                    ...
                    For lnCtr = 0 To poDTTemp.Rows.Count - 1
                        loDR = poDT.Select("nEntryNox = " & strParm(poDTTemp(lnCtr)("nEntryNox")))

                        If loDR.Length = 0 Then
                            lsSQL = "DELETE FROM " & pxeTableName & _
                                    " WHERE sDeptIDxx = " & strParm(psDeptIDxx) & _
                                        " AND nEntryNox = " & strParm(poDTTemp(lnCtr)("nEntryNox"))
                    ...
                    For lnCtr = 0 To poDT.Rows.Count - 1
                        If poDT(lnCtr)("sMobileNo") = "" Then GoTo nextrec
                        ...
                        Else
                            lsSQL = "UPDATE " & pxeTableName & _
                                    " SET sMobileNo = " & strParm(poDT(lnCtr)("sMobileNo")) & _
                                        ", nEntryNox = " & lnCtr + 1 & _
                                    " WHERE sDeptIDxx = " & strParm(psDeptIDxx) & _
...
nextrec:
                ...
                Else
                    lsSQL = "DELETE FROM " & pxeTableName & _
                            " WHERE sDeptIDxx = " & strParm(psDeptIDxx)
    ...
    Private Sub AddDetail()
        ...
        If IFNull(poDT(lnRow)("sMobileNo")) = "" Then Exit Sub

        poDT.Rows.Add()
endProc:
        lnRow = poDT.Rows.Count - 1
        poDT(lnRow)("nEntryNox") = poDT.Rows.Count
        poDT(lnRow)("sMobileNo") = ""
        txtOther01.Focus()
    End Sub
